...
//TEST
async function testCreazioneVC(jsonVC) {
   
  // Inizia il timer
   const startTime = performance.now();

  // Mostra prompt di approvazione di MetaMask
  await requestMetaMaskApproval();
  ...
  console.log(`Tempo di creazione della credenziale: ${timeTaken} ms`);
}

// Funzione per simulare la richiesta di approvazione di MetaMask
async function requestMetaMaskApproval() {
  return new Promise((resolve, reject) => {
      // Simula il tempo di attesa per l'approvazione di MetaMask
      setTimeout(() => {
          resolve();
      }, 2000); // ad esempio 2 secondi
  });
}
